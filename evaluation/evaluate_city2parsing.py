...
def eval_city2parsing(real_dir, fake_dir):

    reals = glob.glob(os.path.join(real_dir ,'*jpg'))
    fakes = glob.glob(os.path.join(fake_dir, '*.png'))
    reals = sorted(reals)
    fakes = sorted(fakes)
    num_imgs = len(reals)
    ...
    return 'pix_acc: {0}, mean_acc: {1}, mean_IoU: {2}'.format(pix_acc, mean_acc, mean_IoU)
