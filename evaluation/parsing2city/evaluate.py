...
caffemodel_dir = './evaluation/parsing2city/caffemodel'
gpu_id = 0
...
def eval_parsing2city(real_dir, fake_dir):

    n_cl = 19
    label_frames = glob.glob(os.path.join(real_dir ,'*jpg'))
    imgs = glob.glob(os.path.join(fake_dir, '*.png'))
    label_frames = sorted(label_frames)
    imgs = sorted(imgs)
    caffe.set_device(gpu_id)
    caffe.set_mode_gpu()
    net = caffe.Net(os.path.join(caffemodel_dir, 'deploy.prototxt'),
                    os.path.join(caffemodel_dir, 'fcn-8s-cityscapes.caffemodel'),
                    caffe.TEST)
    ...
    return 'pix acc: {0},  mean acc: {1},  mean IoU: {2}'.format(mean_pixel_acc, mean_class_acc, mean_class_iou)
