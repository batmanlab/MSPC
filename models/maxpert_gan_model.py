...
class MaxpertGANModel(BaseModel):
    ...
    def forward_perturbation(self):
        self.netG_copy = networks.define_G(self.opt.input_nc, self.opt.output_nc, self.opt.ngf, self.opt.netG, self.opt.norm,
                                        not self.opt.no_dropout, self.opt.init_type, self.opt.init_gain, self.gpu_ids)
        self.netG_copy.load_state_dict(self.netG.state_dict())
        epsilon = 1e-6

        w_perturbation = []

        perturbation = nn.Parameter(torch.randn(self.real.shape, requires_grad=True)).to(self.device)
        perturbation /= (1e-12 + perturbation.abs().max(dim=0, keepdim=True)[0])
        perturbation /= torch.sqrt(1e-6 + torch.sum(perturbation**2, dim= [i for i in range(1, len(perturbation.shape))] ,keepdim=True))
        perturbation *= epsilon
        w_perturbation.append(perturbation)

        for param in self.netG_copy.parameters():
            perturbation = nn.Parameter(torch.ones(param.size(), requires_grad=True)).to(self.device)
            perturbation /= (1e-12 + perturbation.abs().max())
            perturbation /= (1e-6 + (perturbation ** 2).sum()).sqrt()
            perturbation *= epsilon
            w_perturbation.append(perturbation)

        for i, param_copy in enumerate(self.netG_copy.parameters()):
            param_copy.requires_grad = False
            param_copy += w_perturbation[i + 1]

        # for i, param in enumerate(self.netG.parameters()):
        #     if i == 0:
        #         print(param - a)

        fake_perturbation = self.netG_copy(self.real + w_perturbation[0])

        loss_consistency = self.criterionIdt(self.fake.detach(), fake_perturbation)

        gradients = grad(outputs = loss_consistency, inputs = w_perturbation,
                                  grad_outputs= torch.ones(loss_consistency.shape).to(self.device),
                                  create_graph= True, retain_graph=False, only_inputs=True, allow_unused=True)

        self.netG_copy = networks.define_G(self.opt.input_nc, self.opt.output_nc, self.opt.ngf, self.opt.netG, self.opt.norm,
                                        not self.opt.no_dropout, self.opt.init_type, self.opt.init_gain, self.gpu_ids)

        for i, (param, param_copy) in enumerate(zip(self.netG.parameters(), self.netG_copy.parameters())):
            param_copy.requires_grad = False
            param_copy *= 0
            param_copy += self.opt.lambda_pert*gradients[i + 1].detach().data
            param_copy += param

        self.fake_perturbation = self.netG_copy(self.real + 8.0*gradients[0].detach())
