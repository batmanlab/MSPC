...
class Maxgcpert3GANModel(BaseModel):
    ...
    def scale_constraint(self, source_control_points, target_control_points):
        ...
        constraint_dis_t2 = ((constraint_target_points2[:, 0, :] - constraint_target_points2[:, 1, :]) ** 2).sum(
            1).sqrt()
        # constraint_dis = ((source_control_points.unsqueeze(dim=2) - source_control_points.unsqueeze(dim=1)) ** 2).sum(
        #     3)
        # constraint_dis_t = ((self.target_control_points.unsqueeze(dim=2) - self.target_control_points.unsqueeze(dim=1)) ** 2).sum(
        #     3)
        # constraint_dis = constraint_dis[constraint_dis_t!=0]
        # constraint_dis_t = constraint_dis_t[constraint_dis_t!=0]
